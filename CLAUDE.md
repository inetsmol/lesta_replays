...
- `python manage.py runserver` - сервер разработки запускается ТОЛЬКО пользователем вручную

**КРИТИЧЕСКИ ВАЖНО**: Сервер разработки (`python manage.py runserver`) управляется исключительно пользователем.
НЕ пытайтесь запускать, останавливать или перезапускать сервер. Сервер уже запущен и работает.
Для проверки внешнего вида используйте MCP Chrome DevTools для подключения к уже запущенному серверу.
